...
def test(args):
    ...
    mesh_path = os.path.join(conf['dataset'].data_dir, "{}.ply".format(args.dataname))
    ...
    evaluator = dcudf2(lambda pts: udf_network.udf(pts), conf.get_int('evaluate.resolution'), conf.get_float('evaluate.threshold'),
                      ...
                      sub_weight = conf.get_float("evaluate.sub_weight"),
                      local_weight = conf.get_float("evaluate.local_weight"),
                      local_it = conf.get_int("evaluate.local_it"),
                      cut_it = conf.get_int("evaluate.cut_it"),
                      optimization_oc = conf.get_int("evaluate.optimization_oc"),
                      )
